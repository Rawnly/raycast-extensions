...
import { divideNumber, handleResult } from "./lib/utils";
...
export default function SetVolume(props: LaunchProps) {
  ...
  const { data: volume, isLoading, mutate } = useCachedPromise(Result.promisify(scripts.player.volume.get));
  ...
  useEffect(() => {
    ...
    handleResult(() => scripts.player.volume.set(parseInt(props.arguments.volume)), {
      onSuccess: `Volume set to ${props.arguments.volume}`,
    })().then(() => {
      popToRoot();
      closeMainWindow();
    });
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                onAction={handleResult(() => scripts.player.volume.set(step), { onSuccess: `Volume set to ${step}` })}
              />
            </ActionPanel>
          }
        />
